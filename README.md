...
GitHub Actions Cron으로 매일 17:04 KST에 자동 실행. 별도 서버 없음, 인프라 비용 0원.
...
| `NOTION_CALENDAR_DB_ID` | 회의 일정 캘린더 DB ID (선택, 오류 시 실패 안내만 전송) | DB URL의 32자리 UUID |
| `SLACK_WEBHOOK_URL` | Slack Incoming Webhook URL | https://api.slack.com/apps |

### 2. GitHub Secrets 등록

레포 **Settings → Secrets and variables → Actions → New repository secret** 에서 등록. 회의 알림까지 쓰려면 위 4개를 모두 등록.
...
알릴 안건과 회의가 없으면 슬랙으로 메시지 안 보냄 (스팸 방지). 캘린더 조회가 실패하면 실패 안내를 슬랙으로 보냄.
